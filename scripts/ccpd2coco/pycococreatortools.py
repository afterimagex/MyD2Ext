...
def create_annotation_info(annotation_id, image_id, category_info, binary_mask,
    ...
    annotation_info = {
        ...
        "iscrowd": is_crowd,
        # 0或1，指定为0，表示“单个的对象（不存在多个对象重叠）”.只要是iscrowd=0那么segmentation就是polygon格式
        "area": area.tolist(),
        # area of encoded masks，是标注区域的面积。如果是矩形框，那就是高乘宽； 浮点数，需大于0，因icdar数据没有segmentation，所以本项人为指定为10
        "bbox": bounding_box.tolist(),
        "segmentation": segmentation,
        # polygon格式.这些数按照相邻的顺序两两组成一个点的xy坐标，如果有n个数（必定是偶数），那么就是n/2个点坐标。
...
def create_annotation_info_polygon(
        annotation_id,
        image_id,
        area,
        category_id,
    ...
    annotation_info = {
        ...
        # v=0 表示这个关键点没有标注
        # v=1 表示这个关键点标注了但是不可见
        # v=2 表示这个关键点标注了同时也可见 [10, 10, 0, 10, 20, 1]
        "width": image_size[0],
        "height": image_size[1],
    }
